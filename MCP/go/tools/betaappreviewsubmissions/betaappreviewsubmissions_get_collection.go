...
func Betaappreviewsubmissions_get_collectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["filter[betaReviewState]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("filter[betaReviewState]=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v1/betaAppReviewSubmissions%s", cfg.BaseURL, queryString)
		...
		var result models.BetaAppReviewSubmissionsResponse
...
func CreateBetaappreviewsubmissions_get_collectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_betaAppReviewSubmissions",
		mcp.WithDescription(""),
		mcp.WithArray("filter[betaReviewState]", mcp.Description("filter by attribute 'betaReviewState'")),
		mcp.WithArray("filter[build]", mcp.Required(), mcp.Description("filter by id(s) of related 'build'")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Betaappreviewsubmissions_get_collectionHandler(cfg),
	}
}
