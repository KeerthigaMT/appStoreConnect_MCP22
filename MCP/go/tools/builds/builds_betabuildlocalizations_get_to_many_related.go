...
func Builds_betabuildlocalizations_get_to_many_relatedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/builds/%s/betaBuildLocalizations%s", cfg.BaseURL, queryString)
...
func CreateBuilds_betabuildlocalizations_get_to_many_relatedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_builds_id_betaBuildLocalizations",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Builds_betabuildlocalizations_get_to_many_relatedHandler(cfg),
	}
}
