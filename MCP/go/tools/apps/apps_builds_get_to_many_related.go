...
func Apps_builds_get_to_many_relatedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/apps/%s/builds%s", cfg.BaseURL, queryString)
...
func CreateApps_builds_get_to_many_relatedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_apps_id_builds",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_builds_get_to_many_relatedHandler(cfg),
	}
}
