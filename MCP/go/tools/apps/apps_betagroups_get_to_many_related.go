...
func Apps_betagroups_get_to_many_relatedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/apps/%s/betaGroups%s", cfg.BaseURL, queryString)
...
func CreateApps_betagroups_get_to_many_relatedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_apps_id_betaGroups",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_betagroups_get_to_many_relatedHandler(cfg),
	}
}
