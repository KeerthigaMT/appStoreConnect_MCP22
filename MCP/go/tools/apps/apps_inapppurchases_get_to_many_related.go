...
func Apps_inapppurchases_get_to_many_relatedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["filter[inAppPurchaseType]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("filter[inAppPurchaseType]=%v", val))
		}
		if val, ok := args["filter[canBeSubmitted]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("filter[canBeSubmitted]=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v1/apps/%s/inAppPurchases%s", cfg.BaseURL, queryString)
		...
		var result models.InAppPurchasesResponse
...
func CreateApps_inapppurchases_get_to_many_relatedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_apps_id_inAppPurchases",
		mcp.WithDescription(""),
		mcp.WithArray("filter[inAppPurchaseType]", mcp.Description("filter by attribute 'inAppPurchaseType'")),
		mcp.WithArray("filter[canBeSubmitted]", mcp.Description("filter by canBeSubmitted")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_inapppurchases_get_to_many_relatedHandler(cfg),
	}
}
