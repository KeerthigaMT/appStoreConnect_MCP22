...
func Betagroups_get_instanceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/betaGroups/%s%s", cfg.BaseURL, queryString)
...
func CreateBetagroups_get_instanceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_betaGroups_id",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Betagroups_get_instanceHandler(cfg),
	}
}
