...
func Appstoreversions_get_instanceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["limit[appStoreVersionLocalizations]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("limit[appStoreVersionLocalizations]=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v1/appStoreVersions/%s%s", cfg.BaseURL, queryString)
		...
		var result models.AppStoreVersionResponse
...
func CreateAppstoreversions_get_instanceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_appStoreVersions_id",
		mcp.WithDescription(""),
		...
		mcp.WithNumber("limit[appStoreVersionLocalizations]", mcp.Description("maximum number of related appStoreVersionLocalizations returned (when they are included)")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Appstoreversions_get_instanceHandler(cfg),
	}
}
