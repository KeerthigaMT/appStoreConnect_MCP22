...
func Users_get_collectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["filter[username]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("filter[username]=%v", val))
		}
		...
		if val, ok := args["fields[users]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("fields[users]=%v", val))
		}
		...
		var result models.UsersResponse
...
func CreateUsers_get_collectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_users",
		mcp.WithDescription(""),
		mcp.WithArray("filter[roles]", mcp.Description("filter by attribute 'roles'")),
		mcp.WithArray("filter[username]", mcp.Description("filter by attribute 'username'")),
		...
		mcp.WithArray("fields[users]", mcp.Description("the fields to include for returned resources of type users")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_get_collectionHandler(cfg),
	}
}
