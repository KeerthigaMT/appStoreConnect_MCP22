...
func Profiles_get_collectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["filter[profileState]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("filter[profileState]=%v", val))
		}
		if val, ok := args["filter[profileType]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("filter[profileType]=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v1/profiles%s", cfg.BaseURL, queryString)
		...
		var result models.ProfilesResponse
...
func CreateProfiles_get_collectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_profiles",
		mcp.WithDescription(""),
		mcp.WithArray("filter[name]", mcp.Description("filter by attribute 'name'")),
		mcp.WithArray("filter[profileState]", mcp.Description("filter by attribute 'profileState'")),
		mcp.WithArray("filter[profileType]", mcp.Description("filter by attribute 'profileType'")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Profiles_get_collectionHandler(cfg),
	}
}
