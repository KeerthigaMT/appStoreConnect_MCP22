...
func Profiles_certificates_get_to_many_relatedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/profiles/%s/certificates%s", cfg.BaseURL, queryString)
...
func CreateProfiles_certificates_get_to_many_relatedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_profiles_id_certificates",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Profiles_certificates_get_to_many_relatedHandler(cfg),
	}
}
