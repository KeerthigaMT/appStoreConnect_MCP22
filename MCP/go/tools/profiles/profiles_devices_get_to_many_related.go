...
func Profiles_devices_get_to_many_relatedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/profiles/%s/devices%s", cfg.BaseURL, queryString)
...
func CreateProfiles_devices_get_to_many_relatedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_profiles_id_devices",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Profiles_devices_get_to_many_relatedHandler(cfg),
	}
}
