...
func Appscreenshotsets_get_instanceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["limit[appScreenshots]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("limit[appScreenshots]=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v1/appScreenshotSets/%s%s", cfg.BaseURL, queryString)
		...
		var result models.AppScreenshotSetResponse
...
func CreateAppscreenshotsets_get_instanceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_appScreenshotSets_id",
		mcp.WithDescription(""),
		...
		mcp.WithNumber("limit[appScreenshots]", mcp.Description("maximum number of related appScreenshots returned (when they are included)")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Appscreenshotsets_get_instanceHandler(cfg),
	}
}
