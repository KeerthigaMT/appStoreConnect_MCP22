...
func Apppricepoints_get_collectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["filter[priceTier]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("filter[priceTier]=%v", val))
		}
		if val, ok := args["filter[territory]"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("filter[territory]=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v1/appPricePoints%s", cfg.BaseURL, queryString)
		...
		var result models.AppPricePointsResponse
...
func CreateApppricepoints_get_collectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_appPricePoints",
		mcp.WithDescription(""),
		mcp.WithArray("filter[priceTier]", mcp.Description("filter by id(s) of related 'priceTier'")),
		mcp.WithArray("filter[territory]", mcp.Description("filter by id(s) of related 'territory'")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apppricepoints_get_collectionHandler(cfg),
	}
}
